...
**운영자에게 `config.ini` 를 직접 열라고 하지 않는다.** 현장 작업자는 출력 담당이지 파일을
편집하는 사람이 아니다. 원격 지원 중이면 더더욱 그렇다.

- 새 설정을 추가하면 **같은 커밋에서 설정 패널(`gui/settings_panel.py`)에 항목을 넣는다.**
  ini 키만 만들고 끝내지 않는다.
- 문서·안내에 "config.ini 를 이렇게 고치세요" 라고 쓰지 않는다. "설정 > (섹션) > (항목)" 으로
  안내한다.
- ini 는 기본값 정의와 고급 진단용 통로일 뿐, 운영 인터페이스가 아니다.
